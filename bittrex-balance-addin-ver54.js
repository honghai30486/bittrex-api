...
s.type = "text/javascript";
s.src = "https://rawgit.com/honghai30486/bittrex-api/master/bitbank-api-v2.js?ver="+(new Date()).getTime();
$("head").append(s);
		...
		getmarketsummaries(function(marketsummaries){
			getorderhistory("",function(orderhistories){
				b_getticker("btc_jpy", function(bitbankdata){
					...
					message +=   '\n------------------------------------------------------------------------------'
					message +=   '\n∑PROFIT : ' + profit.toFixed(4) + ' (BTC) ⇒ ' + (profit*priceArray["USDT-BTC"]).toFixed(2) + " (USDT) ⇒ "+ (bitbankdata.data.last*profit).toFixed(0) +"(JPY)(>_<)";
